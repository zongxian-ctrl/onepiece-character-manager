// Battle/shop game logic — pure functions, no DB or HTTP. Every function that
// rolls dice takes an rng (defaults to Math.random) so tests can be deterministic.
//
// NOTE: client/src/game.js is an ESM mirror of this file for the static
// (GitHub Pages) build. If you change balance numbers or formulas here,
...
// Stats scale with bounty (log scale: 3B ≈ max power) plus a random spread,
// so big names are genuinely dangerous while rookies stay beatable.
function rollStats(bounty, rng = Math.random) {
  const power =
    bounty > 0 ? Math.min(1, Math.log10(bounty) / 9.6) : 0.05;
...
function computeDamage(attack, defense, rng = Math.random) {
  const raw = attack * (0.85 + 0.3 * rng()) - defense * 0.65;
  return Math.max(4, Math.round(raw));
}

// One round of combat: the player strikes; if the target survives, it strikes
// back. Mutates copies, never the inputs. Returns null character/player fields
// merged by the caller into storage.
function resolveAttack(playerIn, characterIn, rng = Math.random) {
  if (characterIn.hp <= 0) {
...
// The catalog is derived from the player: training prices escalate with each
// purchase, weapon/armor show the next tier (or sell out at max).
function shopCatalog(player) {
...
// Applies a purchase to a copy of the player. Returns { player, purchased }
// on success or { error } on failure — never throws.
function applyPurchase(playerIn, itemId) {
